...
export async function generateMetadata(): Promise<Metadata> {
  const { metadata } = await readContentFile('acerca-de', 'institucion-patrocinante');
  return {
...
export default async function InstitucionPatrocinantePage() {
  const { content, metadata } = await readContentFile('acerca-de', 'institucion-patrocinante');
  ...
  const imageAlt = metadata.image ? metadata.title : 'Guías y Scouts Nua Mana';

  return (
    <>
      <MdNavbar currentPage={metadata.title || 'Institucion Patrocinante'} />
