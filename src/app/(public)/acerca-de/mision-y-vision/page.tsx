...
export async function generateMetadata(): Promise<Metadata> {
  const { metadata } = await readContentFile('acerca-de', 'mision-y-vision');
  return {
    title: `${metadata.title} | Guías y Scouts Nua Mana`,
    description: metadata.description || 'Información acerca de Guías y Scouts Nua Mana',
  };
}

export default async function MisionVisionPage() {
  const { content, metadata } = await readContentFile('acerca-de', 'mision-y-vision');
  ...
  return (
    <>
      <MdNavbar currentPage={metadata.title || 'Mision Y Vision'} />
