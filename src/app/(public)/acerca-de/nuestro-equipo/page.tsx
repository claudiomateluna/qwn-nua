...
export async function generateMetadata(): Promise<Metadata> {
  const { metadata } = await readContentFile('acerca-de', 'nuestro-equipo');
  return {
...
export default async function NuestroEquipoPage() {
  const { content, metadata } = await readContentFile('acerca-de', 'nuestro-equipo');
  ...
  return (
    <>
      <MdNavbar currentPage={metadata.title || 'Nuestro Equipo'} />
