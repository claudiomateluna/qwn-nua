...
export async function generateMetadata(): Promise<Metadata> {
  const { metadata } = await readContentFile('acerca-de', 'nuestra-historia');
  return {
...
export default async function NuestraHistoriaPage() {
  const { content, metadata } = await readContentFile('acerca-de', 'nuestra-historia');
  ...
  return (
    <>
      <MdNavbar currentPage={metadata.title || 'Nuestra Historia'}/>
