...
export async function generateMetadata(): Promise<Metadata> {
  const { metadata } = await readContentFile('acerca-de', 'quienes-somos');
  return {
...
export default async function QuienesSomosPage() {
  const { content, metadata } = await readContentFile('acerca-de', 'quienes-somos');
  ...
  return (
    <>
      <MdNavbar currentPage={metadata.title || 'Quienes Somos'} />
