...
export async function generateMetadata(): Promise<Metadata> {
  const { metadata } = await readContentFile('lo-que-hacemos', 'el-metodo-scout');
  return {
...
export default async function ElMetodoScoutPage() {
  const { content, metadata } = await readContentFile('lo-que-hacemos', 'el-metodo-scout');
  ...
  return (
    <>
      <MdNavbar currentPage={metadata.title || 'El Metodo Scout'} />
