...
export async function generateMetadata(): Promise<Metadata> {
  const { metadata } = await readContentFile('lo-que-hacemos', 'vida-reflexiva');
  return {
...
export default async function VidaReflexivaPage() {
  const { content, metadata } = await readContentFile('lo-que-hacemos', 'vida-reflexiva');
  ...
  return (
    <>
      <MdNavbar currentPage={metadata.title || 'Vida Reflexiva'} />
