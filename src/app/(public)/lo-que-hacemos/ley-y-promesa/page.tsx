...
export async function generateMetadata(): Promise<Metadata> {
  const { metadata } = await readContentFile('lo-que-hacemos', 'ley-y-promesa');
  return {
...
export default async function LeyYPromesaPage() {
  const { content, metadata } = await readContentFile('lo-que-hacemos', 'ley-y-promesa');
  ...
  return (
    <>
      <MdNavbar currentPage={metadata.title || 'Ley Y Promesa'} />
