...
export async function generateMetadata(): Promise<Metadata> {
  const { metadata } = await readContentFile('lo-que-hacemos', 'aprender-haciendo');
  return {
...
export default async function AprenderHaciendoPage() {
  const { content, metadata } = await readContentFile('lo-que-hacemos', 'aprender-haciendo');
  ...
  return (
    <>
      <MdNavbar currentPage={metadata.title || 'Aprender Haciendo'} />
